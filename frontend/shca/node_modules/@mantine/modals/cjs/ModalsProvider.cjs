...
function ModalsProvider({ children, modalProps, labels, modals }) {
  ...
  const closeContextModal = react.useCallback(
    (modalKey, canceled) => {
      const id = stateRef.current.modals.find((m) => m.type === "context" && m.ctx === modalKey)?.id ?? modalKey;
  ...
  const updateContextModal = react.useCallback(
    ({ modalKey, modalId, ...newProps }) => {
      const id = modalId ?? stateRef.current.modals.find((m) => m.type === "context" && m.ctx === modalKey)?.id ?? modalKey;
